// 4KHDHub provider for Nuvio — Hikari patched build.
// Changes vs upstream:
//  - Modern mobile browser User-Agent (upstream sent Chrome/91 desktop, which
//    the site's Cloudflare firewall rejects with "Access denied" before any
//    HTML is returned — so the catalog page never matched and every title
//    failed with "no sources").
//  - Browser Accept / Accept-Language / matching-Origin Referer headers on
//    every page fetch (the WAF requires a plausible browser request).
//  - The dynamic-domain lookup (phisher98/TVVVV domains.json, which currently
//    points 4khdhub at https://4khdhub.one) is unchanged.
"use strict";
...
var USER_AGENT = "Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Mobile Safari/537.36";
